...
-- 윈도우 > 설정 > 연결 > 연결유형 Auto commit by derault 체크 해제 되어있는지 다시 확인

-- ** 테이블을 복사하면 DEFAULT 값과 COMMENT는 복사가 안된다. 테이블 값과 형식만 복사됨 **
...
VALUES('900', '홍길동', '941231-1312457', 'hong_gildong@or.kr', '01012341234',
...
-- DDL 수행시 바로 COMMIT 됨(영구저장)
-- CREATE 할 때 바로 COMMIT 된다.
...
VALUES('900', '홍길동', '941231-1234567', 'hong_gildong@or.kr', '01012344567',
'D1', 'J7', 'S3', 4300000);
...
-- DEPARTMENT2 테이블에서 DEPT_ID 가 'D9'인 부서의
...
-- EMPLOYEE2 테이블에서 BOUNUS 를 받지 않는 사원의
-- BONUS를 0.1로 변경
SELECT * FROM EMPLOYEE2;
...
UPDATE DEPARTMENT2 
SET DEPT_ID = 'D0', DEPT_TITLE = '전략기획팀' -- WHERE 절 직전 컬럼에는 콤마 작성 X
WHERE DEPT_ID = 'D9'
AND DEPT_TITLE = '총무부';
...
'01012345612', 'D9', 'J4', 'S1', 9000000, 0.5, NULL, SYSDATE, NULL, 'N');
...
-- 5. TRUNCATE (DML 아님. DDL 입니당)
-- 테이블의 전체 행을 삭제하는 DDL
-- DELETE 보다 수행속도 더 빠름
-- ROLLBACK 을 통해 복구 불가능
