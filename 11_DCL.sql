...
CREATE USER jma_sample IDENTIFIED BY 1234;


-- 2. 새 연결(접속) 추가
-- ORA-01045: 사용자 JMA_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한

-- 3. 접속 권한 부여(sys -> 사용자 계정)
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한... TO 사용자명
GRANT CREATE SESSION TO jma_sample;
...
GRANT CREATE TABLE TO jma_sample;

ALTER USER jma_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- RESOURCE : DB 사용을 위한 기본 객체생성 권한을 묶어둔 ROLE
--> CREATE TABLE, CREATE SEQUENCE...

-- (sys) sample 계정에 CONNECT, RESOURCE ROLE 부여
GRANT CONNECT, RESOURCE TO jma_sample;
...
GRANT SELECT ON EMPLOYEE TO jma_sample;

-- 3. (sample) kh 계정이 가진 EMPLOYEE 테이블 조회
...
REVOKE SELECT ON EMPLOYEE FROM jma_sample;

-- 5. (sample) EMPLOYEE 테이블 조회해보기
SELECT * FROM KH.EMPLOYEE; --> 조회 안됨. 권한 없음.
