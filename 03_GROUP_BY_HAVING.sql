...
-- 그룹함수는 단 한개의 결과값만 산출하기 때문에 드룹이 여러개일 경우 오류 발생
...
SUM(SALARY) "급여 합"
/*1*/FROM EMPLOYEE
/*2*/GROUP BY DECODE(SUBSTR(EMP_NO, 8,1), '1', '남', '2', '여') -- 별칭 사용X(SELECT 절 해석 X)
/*4*/ORDER BY "인원 수"; -- 별칭사용O(SELECT절 해석 완료)

-----------------------------------------------------------------

-- * WHERE 절 GROUP BY 절 혼합하여 사용하기 *
--> WHERE 절은 각 컬럽값에 대한 조건
--> HAVING 절은 그룹에 대한 조건

-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서코드가 D5, D6인 부서의(WHERE절 -> GROUP BY 절)
...
-- EMPLOYEE 테이블에서
-- 부서별 평균 급여가 300만원 이상인 부서워
-- 부서코드, 평균급여 조회
-- 부서코드 오름차순
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 --> 한 사람의 급여갸 300만원 이상이라는 조건(요구사항X)
GROUP BY DEPT_CODE
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중 급여 평균이 300만원 이상인 그룹만 조회
-- 이미 그룹화 된 상태, 그룹별로 급여평균을 내는것
ORDER BY DEPT_CODE;
...
-- ROLLUP : GROUP BY 절에서 가낭 먼저 작성된 컬럼의 중간집계를 처리하는 함수
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
 ...
 * 
 * - UNION ALL : UNION + INTERSECT 합집합에서 중복부분을 제거
 * 
...
-- (주의사항!) 집합연산자를 사용하기 위한 SELECT 문들은
