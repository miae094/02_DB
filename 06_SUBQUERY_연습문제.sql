...
JOB_NAME, TO_CHAR(HIRE_DATE, 'YY/MM/DD')
FROM EMPLOYEE MAIN
JOIN JOB USING (JOB_CODE)
...
-- 나이가 가장 어린 직원
SELECT MIN((EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(EMP_NO, 1,2)+1900 +1)))
FROM EMPLOYEE;

-- 메인쿼리 : 사번, 이름, 직급명, 나이, 보너스 포함 연봉을 조회하고
-- 나이순으로 내림차순 정렬하세요
SELECT EMP_ID, EMP_NAME, JOB_NAME, (EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(EMP_NO, 1,2)+1900 +1)) 나이, (SALARY*12 + SALARY*BONUS) 보너스포함연봉
FROM EMPLOYEE MAIN
JOIN JOB USING (JOB_CODE)
ORDER BY 나이 DESC;
...
WHERE EMP_ID = (SELECT MIN((EXTRACT(YEAR FROM SYSDATE) - (SUBSTR(EMP_NO, 1,2)+1900 +1)))
								FROM EMPLOYEE SUB;
								WHERE MAIN.EMP_ID = SUB.EMP_ID)		
ORDER BY 나이 DESC;
