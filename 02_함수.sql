...
-- 함수는 SELECT 문의
-- SELECT절, WHERE절, ORDER BY 절, GROUP BY절, HAVING 절 사용 가능
...
-- 문자열을 앞에서 부터 검색하여 첫번째 B의 위치를 조회
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL;

-- 문자열을 다섯번째 문자부터 검색하여 첫번째 B위치 조회
...
SELECT SYSDATE FROM DUAL; --2025-03-10 12:11:40.000

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL; -- 2025-03-10 12:12:12.818 +0900
...
-- 문자열로 변환
-- TO_CHAR(날짜, [포멧]) : 날자형 데이터를 문자형 데이터로 변경
-- TO_CHAR(숫자, [포맷]) : 숫자형 데이터를 문자형 데이터로 변경

-- 숫자 -> 문자 변환 시 포맷 패턴
-- 9 : 숫자 한 칸을 의미, 여러개 작성시 오른쪽 정렬
-- 0 : 숫자 한 칸을 의미, 여러개 작성시 오른쪽 정렬 + 빈칸이 있다면 0으로 채움
...
SELECT TO_DATE('20250310 140730', 'YYYYMMDD HH24MISS') FROM DUAL;
...
SELECT EMP_NAME, TO_DATE(SUBSTR(EMP_NO, 1, INSTR(EMP_NO, '-') -1), 'RRMMDD') AS 생년월일
FROM EMPLOYEE;
...
-- 직원 성별 구하기
SELECT DECODE(SUBSTR(EMP)NO, 8,1), '1', '남성', '2','여성') 성별
FROM EMPLOYEE;
...
SELECT SUM(SALARY) 급여합계, ROUND(AVG(SALARY)) 급여평균
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
