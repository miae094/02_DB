...
SELECT * FROM USER_TABLES; -- 데이터 딕셔너리 뷰
SELECT * FROM USER_CONSTRAINTS; -- 제약조건
SELECT * FROM USER_CONS_COLUMNS; -- 제약 조건이 적용되어있는 컬럼에 대한 정보를 보여주는 딕셔너리 뷰
...
-- MEMBER 테이블 생성
CREATE TABLE "MEMBER" ( -- 쌍따옴표 붙일경우 대소문자 그대로 표기됨
	MEMBER_ID VARCHAR2(20),
	MEMBER_PWD VARCHAR2(20),
	MEMBER_NAME VARCHAR2(30),
	MEMBER_SSN CHAR(14), --'990313-1234567'
	ENROLL_DATE DATE DEFAULT SYSDATE
);
-- Updated Rows -> 행과 관련된 것이 변경됐을때만 숫자 나옴
...
-- 테이블 생성 시 해당 컬럼에 지정된 DEFAULT 값으로 삽입됨 *
...
-- 가입일 -> SYSDATE로 삽입
INSERT INTO "MEMBER" VALUES('MEM02', 'QWER1234', '김사랑',
'010101-2245784', SYSDATE);
-- DEFAULT 설정된 컬럼이라고 삽입/수정시 DEFAULT 만 작성해야 하는 것 아님!
-- 컬럼 데이터 타입에 맞춘 데이터 삽입/수정 가능


-- 가입일 -> INSERT 시 미작성 하는 경우 DEFAULT 값이 반영되는지 확인
-- INSERT INTO 테이블명 (컬럼명1, 컬럼명2) VALUES(값1, 값2);
INSERT INTO "MEMBER" (MEMBER_ID, MEMBER_PWD, MEMBER_NAME)
VALUES ('MEM03', '1234ZZZ', '이지연');
...
--> 전화번호나 주민등록번호처럼 숫자로만 되어있는 데이터라도
...
INSERT INTO USER_USED_PK VALUES(
1, 'USER02', 'PASS02', '최고민', '남자',
'010-1234-0545', 'choi_gd@or.kr');
...
INSERT INTO USER_USED_PK VALUES(
NULL, 'USER02', 'PASS02', '최고민', '남자',
'010-1234-0545', 'choi_gd@or.kr');
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
--> 기본키 NULL이므로 오류!

INSERT INTO USER_USED_PK VALUES(
2, 'USER02', 'PASS02', '최고민', '남자',
'010-1234-0545', 'choi_gm@or.kr');
...
1, 'USER02', 'PASS01', '홍길동', '남자',
'010-1234-4567', 'hong_gd@or.kr');
-- 둘다 중복되었을때만 제약조건 위배. 둘중 하나라도 달라서 위배 아님
...
CREATE TABLE USER_GRADE (
	...
	GRADE_MANE VARCHAR2(30) NOT NULL -- 등급명
...
CREATE TABLE USER_USED_FK (
	USER_NO NUMBER PRIMARY KEY, -- 사용자번호(고유한 번호 : 중복값X / NULL X)
	...
	-- REFERENCES USER_GRAD
	-- FOREIGN KEY 라는 단어는 테이블 레벨엣어만 사용!
...
1, 'USER01', 'PASS01', '홍길동', '남자',
'010-1234-4567', 'hong_gd@or.kr', 10);
...
'010-8524-4241', 'lee_ss@or.kr', 10);
-- USER_GRADE(부모/참조테이블)에 10이라는
-- GRADE_CODE가 존재하므로 가능


INSERT INTO USER_USED_FK VALUES(
3, 'USER03', 'PASS03', '이지금', '여자',
'010-4457-2089', 'lee_now@or.kr', 30);
-- USER_GRADE(부모/참조테이블)에 10이라는
-- GRADE_CODE가 존재하므로 가능

INSERT INTO USER_USED_FK VALUES(
4, 'USER04', 'PASS04', '나재민', '남자',
'010-0825-0411', 'na_jm@or.kr', NULL);
--> NULL도 사용가능 (제공되는 값 외에도 NULL 사용 가능)


INSERT INTO USER_USED_FK VALUES(
5, 'USER05', 'PASS05', '이해찬', '남자',
'010-0606-0825', 'lee_hc@or.kr', 50);
...
-- ON DELETE SET NULL 삭제옵션이 적용된 자식테이블
...
-- ON DELETE CASCADE 삭제옵션이 적용된 자식테이블
...
-- 부모테이블은 USER_GRADE3에서 GRADE_CODE = 10삭제
-- ON DELETE CASCADE 옵션이 설정되어 있어
-- 부모, 부모를 참조하던 자식까지 삭제되어
-- 오류없이 수행
DELETE FROM USER_GRADE3 WHERE GRADE_CODE = 10;


SELECT * FROM USER_GRADE2; -- 부모에서 10 삭제됨
...
INSERT INTO USER_USED_CHECK VALUES(
2, 'USER02', 'PASS02', '한지아', '여',
'010-2587-4575', 'han_ja@or.kr');
-- 가능
