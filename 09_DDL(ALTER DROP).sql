 ...
 * 
 * -- 테이블에서 수정할 수 있는 것
 * 1) 제약 조건(추가/삭제) 수정은 안됨. 잘못만들었을때는 삭제 후 추가해줘야함
 * 2) 컬럼(추가/수정/삭제) 이미 만들어진 테이블에 데이터가 많이 들어있다면 컬럼 추가하는것은 지양
 * 3) 이름변경 (테이블명, 컬럼명..)
 * 
 * 
 * */
...
-- DEPARTMENT 테이블 복사 (컬럼명, 데이터타입, NOT NULL 제약조건만 복사)
...
-- DEPT_COPY의 DEPT_TITLE 컬럼에서 UNIQUE 삭제
ALTER TABLE DEPT_COPY
...
SELECT * FROM DEPT_COPY;
--> !! 기본 값을 변경했다고 해서 기존데이터가 변하지는 않음
--> 앞으로 INSERT 될 데이터만 변경된 내용으로 삽입
...
-- DROP TABLE 테이블명 [CASCAED CONSTRAINTS];
...
--> DDL은 수행 시 존재하고 있는 트랜잭셔늘 모두 DB에 강제 COMMIT 시킴
